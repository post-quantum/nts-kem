...
#define UINT64_SIZE             2
#define BLOCK_SIZE              128
#define NTS_KEM_PARAM_BC_VEC    NTS_KEM_PARAM_BC_DIV_128
#define NTS_KEM_PARAM_R_VEC     NTS_KEM_PARAM_R_DIV_128
#define NTS_KEM_PARAM_N_VEC     NTS_KEM_PARAM_N_DIV_128

#define bitslice_fft    bitslice_fft13_128
#define vector_ff_or    vector_ff_or_128
...
int compute_syndrome(const NTSKEM* nts_kem, const vector *c_ast, ff_unit* s);
                        ...
                        uint8_t *k_r)
{
    ...
    __m128i vec_syndromes[3][NTS_KEM_PARAM_M] = {{{0}}};
    __m128i sigma[2][NTS_KEM_PARAM_M];
    vector evals[NTS_KEM_PARAM_N_VEC][NTS_KEM_PARAM_M];
    vector error[NTS_KEM_PARAM_N_VEC];
    vector allones = _mm_set1_epi64x(-1);
    ...
    uint8_t c_prime[NTS_KEM_CIPHERTEXT_SIZE] __attribute__((aligned(16)));
    ...
    for (i=0; i<NTS_KEM_PARAM_N_VEC; i++) {
        ...
        error[i] = _mm_xor_si128(error[i], allones);
                            ...
                            ff_unit *h)
{
    ...
    for (j=0; j<NTS_KEM_PARAM_M; j++) {
        ...
        for (i=0; i<NTS_KEM_PARAM_N; i++) {
			...
			switch ((l & 127) >> 6) {
				case 1:
					v[i >> 6] |= (((_mm_extract_epi64(vh[l >> 7][j], 1) & (1ULL << (l & 63))) >> (l & 63)) << (i & 63));
					break;
				default:
					v[i >> 6] |= (((_mm_extract_epi64(vh[l >> 7][j], 0) & (1ULL << (l & 63))) >> (l & 63)) << (i & 63));
			}
        }
        for (i=0; i<NTS_KEM_PARAM_N_VEC; i++) {
            hv0[i][j] = _mm_set_epi64x(v[2*i + 1], v[2*i]);
        }
    }
    ...
    vector_store_2d_128(h, (const __m128i (*)[])hv1, NTS_KEM_PARAM_N);
    
    /* Prepare vector a for vectorised multiplication */
    vector_load_2d_128(av0, a, NTS_KEM_PARAM_N);
                ...
                uint8_t *k_r)
{
     ...
     **/
    for (i=0; i<NTS_KEM_PARAM_K; i++) {
        Q[i][NTS_KEM_PARAM_R_VEC-1] = _mm_setzero_si128();
